var autoComplete = angular.module("autoCompleteApp", []);

autoComplete.controller("MainCtrl", ['$scope', 'autoSearch', function($scope, autoSearch) {

	$scope.q = "";

	$scope.suggestions = [];

	// This will do the work of searching for words
	// and inserting them into the suggestion box
	$scope.doAutoComplete = function() {
		// $scope.suggestions = ['apple','appie','appster'];

		console.log("$scope.q: " + $scope.q);

		$scope.suggestions = [];
		$scope.suggestions = autoSearch.all($scope.q);
	};

}]);

autoComplete.directive("wdiAutocomplete", function factory() {

	var searchResults = {
		restrict: 'E',
		templateUrl: 'wdi-autocomplete.html'
	};
...
autoComplete.service("autoSearch", [function () {
	// ------------- PUT TRIE CODE HERE -------------
	...
	// character into the trie. Word boundaries
	...
	Trie.prototype._addWord = function(word,i) {
		...
		if (i == word.length) {
			// use $ word boundary with the simplest
			// truthy value 1.
			this.$ = 1;
	...
	// Mock the data with an array
	var wordData = ['app', 'apples', 'appster'];
	...
	wordData.forEach(function(word) {
		trieInstance.addWord(word);
	});

	// Return the result of the Trie
	return {
		all: function(q) {
			console.log("q: " + q);
			console.log("trieInstance.lookup(q): " + trieInstance.lookup(q));

			return trieInstance.lookup(q);
		}
	}
